...
- [2026-09-01 09:xx] **stock-manager 그리드 매매 분봉 차트 미표시(재발 빈도
  ...
  **일/주/월봉(1d/1w/1M) 분기는 그대로 Vercel에 남음** — 네이버 fchart 단일
  호출이라 이 문제 대상 아님.
- [2026-09-09 08:5x] `coin_grid_jobs.json` (Gist) — 리플(KRW-XRP) 그리드
  격자 2개(1868/1897원 레벨) `sell_waiting`+`sell_uuid=""`(무방비 유령
  물량) 상태를 `idle`로 수동 초기화. 근본원인: 재초기화 이월물량 매도
  재등록 실패 시 상태만 `sell_waiting`으로 남고 실제 주문(`sell_uuid`)은
  없는 채 영구 방치되는 버그 — `process_grid()`가 조용히 건너뛰기만 하고
  `_sync_orphan_coins()`도 state만 보고 "추적중"으로 오판해 못 잡음. 결과:
  실거래소엔 매도 주문이 전혀 없는데 그때 남은 stale `last_sell_price`가
  프론트 차트에 영구 표시(사용자 리포트: "차트에 현재가보다 낮은가격에
  매도로 표기됨"). 코드 근본 수정 후 배포·재시작했더니 데몬이 실제로
  재등록을 시도했으나 `insufficient_funds_ask`로 실패 — 계좌 실측 결과
  XRP 보유량(103.71개) 전부가 이미 진짜 매도주문 2건에 물려있어 이 2개
  격자에 대응하는 실물 코인이 애초에 없는 유령 장부였음을 확인, PATCH로
  수동 정리. 관련 커밋: `8fe2e41`.
