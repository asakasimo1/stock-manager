...
def main():
    ...
    nxt_aftermarket = kis_api._is_nxt_aftermarket()
    if nxt_mode:
        session_label = "NXT 애프터마켓" if nxt_aftermarket else "NXT 프리마켓"
    else:
        session_label = "KRX 정규"
    logger.info("사이클 잡 체크 시작 [%s]", session_label)
